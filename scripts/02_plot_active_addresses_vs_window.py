...
for token in TOKEN_ORDER:
    ...
    y = [subset.loc[w, "active_addresses"] for w in WINDOW_ORDER]
    ax.plot(
        WINDOW_ORDER, y,
...
ax.set_ylabel("Number of active senders (log scale)", fontsize=11)
ax.set_title(
    "Active senders across five Ethereum tokens\n(sent-only definition)",
    fontsize=12, pad=12,
)
...
plt.tight_layout()
plt.savefig(CHARTS_DIR / "02_active_addresses_vs_window.png", dpi=160, bbox_inches="tight")
