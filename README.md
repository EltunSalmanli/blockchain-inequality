...
**Student:** Eltun Salmanli (FJULHI)  **Supervisor:** Seres István András

## Research question

How does measured wealth inequality on Ethereum change when we
restrict the holder set to *active* addresses (those that transacted
in a recent window) rather than all historical holders, and how does
that inequality evolve over time?

## Tokens analyzed

DAI, USDT, USDC, UNI, AAVE
...
**Phase 1 — cross-sectional.** For each token T and each window
W ∈ {1d, 1w, 1m, 3m, 6m, 1y, all-time}, the active set A_T(W) is the set
of addresses that were active in the last W. Two definitions of "active"
are reported side by side: *sent-only* (the address appeared as the
sender of at least one transfer) and *sent-or-received* (the address
appeared as sender or recipient). The Gini coefficient is then computed
on the current balances of A_T(W).

**Phase 2 — temporal.** For AAVE, UNI and DAI the active-holder Gini is
recomputed at 22 quarterly snapshots from 2021 through 2026, for windows
{1d, 1w, 1m, 1y}, tracking how inequality evolves over time. This phase
uses the sent-or-received definition.

The Nakamoto and Palma coefficients are noted as future extensions and
...
- **Cross-token comparison takes shape:** governance tokens (UNI, AAVE) display strong window-sensitivity, while stablecoins (DAI, USDC, USDT) display window-insensitivity.
...
- The all-time `sent_or_received` count is **440,001** — consistent with the ~250,000 wallets that received the 2020 UNI airdrop plus subsequent recipients. A substantial fraction of these never appear in shorter active windows.

**Temporal phase (Queries 08–10):**

- For AAVE, UNI and DAI the active-holder Gini was recomputed at 22 quarterly snapshots (2021–2026) across the 1d/1w/1m/1y windows.
- DAI sits above the two governance tokens in every window at every snapshot; the ordering never inverts across five years.
- UNI's short-window Gini drifts downward over time (a broadening intraday trader base), while DAI drifts gently upward.
- A sharp excursion of AAVE's 1-year Gini toward 1.0 from late 2024 appears only in the 1-year window and is a single-large-holder measurement artefact, not a genuine concentration event.

**Overall observation:** Within the active (sent-only) definition, Gini rises monotonically as the activity window expands — the recently-active holder set shows *less* inequality than the full all-time sender set. Including the never-sending dormant tail (the difference between sent-or-received and sent-only) *lowers* the measured Gini, because those addresses hold comparatively uniform allocations. The two active-definitions produce very similar Gini values, indicating robustness to this definitional choice.
